<template>
	<div style="height: 100%;overflow: scroll;" ref="visitWrap">
		...
		<div class="box-0-wrap">
			<div class="box-0">
				...
				<div class="box-2-wrap">
					<div class="p-3">
						<div class="p-4" style="padding: 10px 0;" v-show="this.$route.params.id == 'sh'">
...
//					}
				...
				this.dateRang2 = this.$store.getters[storeGetName][attr].dateRang;
			...
			lcs1(attr) {
				...
				//根据选择的dateType的不同（周/月/年），发送对应的参数
				let reqParams = {
				...
				if (this.$route.params.id == 'dt' && this.$route.name == 'visit') {
				...
				} else if (this.$route.params.id == 'sh' && this.$route.name == 'visit') {
				...
				} else if (this.$route.params.id == 'xjp' && this.$route.name == 'visit') {
		...
		activated() {
			...
			this.handleData()
			//设置滚动距离
			this.$refs.visitWrap.scrollTop = this.$store.getters.getVisitScrollTop;
		},
		watch: {
			'$route'(to, from) {
				if (to.name == 'visit' && from.name == 'visit') {
					this.handleData()
					//console.log("watch v")
		...
		beforeRouteLeave(to, from, next) {
			//记录滚动距离
			this.$store.dispatch('changeVisitScrollTop', this.$refs.visitWrap.scrollTop)
			next();
		}
